    ...
    *,
    ...
    with tqdm(classified_files, desc=f"FP-FIX {sensor_mode.upper()}", unit="tile", dynamic_ncols=True) as pbar:
        ...
        for idx, gc_fp in enumerate(pbar, start=1):
            ...
            pbar.set_postfix_str(f"{idx}/{len(classified_files)} | {ema_dt:.2f}s/tile | current={Path(gc_fp).name}")

    total_elapsed = perf_counter() - total_t0
    total_changed = int(sum(int(r["changed_points"]) for r in summary_rows))
    total_promoted = int(sum(int(r["promoted_to_ground"]) for r in summary_rows))
    total_demoted = int(sum(int(r["demoted_to_nonground"]) for r in summary_rows))
