...
def _write_tif(grid: np.ndarray, out_fp: str, xmin: float, ymax: float, res: float, crs=None):
    if rasterio is None:
...
def _quick_demote_high_ground_cells(x: np.ndarray, y: np.ndarray, z: np.ndarray, ground: np.ndarray, cell: float) -> np.ndarray:
    ...
    for yy in range(ny):
        ...
        for xx in range(nx):
            ...
            if local.size < 3:
                continue
            # exclude self by replacing one matching element if possible
...
def _load_product_context(classified_fp: str):
    las, x, y, z, cls = _read_las(classified_fp)
    crs = None
    try:
...
def _write_normalized(classified_fp: str, product_dirs: dict[str, str], grid_res: float, dem_method: str = "min"):
    ctx = _load_product_context(classified_fp)
    dem_pack = _build_dem_bundle(ctx, grid_res, dem_method=dem_method)
    z_dem = _sample_bilinear_from_grid(dem_pack["dem"], ctx["x"], ctx["y"], dem_pack["xmin"], dem_pack["ymax"], grid_res)
    z_norm = ctx["z"] - z_dem
...
def _write_chm(
    ...
    dem_pack = _build_dem_bundle(ctx, grid_res, dem_method=dem_method)
    dsm_pack = _build_dsm(ctx, grid_res, dsm_method=dsm_method)
    chm = np.maximum(dsm_pack["dsm"] - dem_pack["dem"], 0.0).astype(np.float32)
...
# ============================================================
# current classifier (stable + targeted void recovery)
# ============================================================
...
def _run_phase(
    ...
    print(f"[TIME] {desc}: {len(items)} tiles | total={total_dt:.2f}s | avg={total_dt / max(len(items), 1):.2f}s/tile")
    return results
...
def derive_products_from_classified_root(
    ...
    if PRODUCT_NORMALIZED in requested_products:
        _run_phase(
            classified_files,
            desc="FAST-GC post-fix NORMALIZED",
            sensor_mode="POST",
    ...
    if PRODUCT_DSM in requested_products:
        _run_phase(
            classified_files,
            desc="FAST-GC post-fix DSM",
            sensor_mode="POST",
            worker=lambda gc_fp: _write_dsm(gc_fp, product_dirs, grid_res, dsm_method=dsm_method),
        )

    if PRODUCT_CHM in requested_products:
        _run_phase(
            classified_files,
            desc="FAST-GC post-fix CHM",
            sensor_mode="POST",
            worker=lambda gc_fp: _write_chm(
                gc_fp, product_dirs, grid_res,
                dem_method=dem_method, dsm_method=dsm_method,
                chm_median_size=chm_median_size, chm_min_height=chm_min_height,
            ),
        )
...
# ============================================================
# batch processing + derivative products
# ============================================================
...
def process_fastgc_path(
    ...
    if Path(in_path).is_file():
        ...
        if PRODUCT_DSM in requested_products:
            _write_dsm(gc_fp, product_dirs, grid_res, dsm_method=dsm_method)
        if PRODUCT_CHM in requested_products:
            _write_chm(
                gc_fp, product_dirs, grid_res,
                dem_method=dem_method, dsm_method=dsm_method,
    ...
    classified_files = _run_phase(
        files,
        desc=f"FAST-GC phase 1/5 CLASSIFY {sensor_mode}",
    ...
    if PRODUCT_DEM in requested_products:
        _run_phase(
            classified_files,
            desc=f"FAST-GC phase 2/5 DEM {sensor_mode}",
            sensor_mode=sensor_mode,
            worker=lambda gc_fp: _write_dem(gc_fp, product_dirs, grid_res, dem_method=dem_method),
        )
    ...
    if PRODUCT_NORMALIZED in requested_products:
        _run_phase(
            classified_files,
            desc=f"FAST-GC phase 3/5 NORMALIZED {sensor_mode}",
            sensor_mode=sensor_mode,
            worker=lambda gc_fp: _write_normalized(gc_fp, product_dirs, grid_res, dem_method=dem_method),
        )
    ...
    if PRODUCT_DSM in requested_products:
        _run_phase(
            classified_files,
            desc=f"FAST-GC phase 4/5 DSM {sensor_mode}",
            sensor_mode=sensor_mode,
            worker=lambda gc_fp: _write_dsm(gc_fp, product_dirs, grid_res, dsm_method=dsm_method),
        )
    ...
    if PRODUCT_CHM in requested_products:
        _run_phase(
            classified_files,
            desc=f"FAST-GC phase 5/5 CHM {sensor_mode}",
            sensor_mode=sensor_mode,
            worker=lambda gc_fp: _write_chm(
                gc_fp, product_dirs, grid_res,
                dem_method=dem_method, dsm_method=dsm_method,
    ...
    print(f"[TIME] FAST-GC TOTAL {sensor_mode}: {len(files)} input tiles | total={total_dt:.2f}s | avg={total_dt / max(len(files), 1):.2f}s/tile")
    return out_root
