...
"""Frozen FAST-GC 0.2.1 CHM implementation.

This module is the behavior-preservation layer created during the CHM package
migration. Existing algorithms remain byte-for-byte logically equivalent to
the former ``fastgc/chm.py`` implementation; only package-relative imports
changed. New CHM algorithms should NOT be added here.

New development belongs in ``fastgc.chm.surfaces`` and is registered through
``fastgc.chm.registry``.
"""
